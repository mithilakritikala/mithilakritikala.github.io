...
const Counter = ({ end, duration, suffix = "", prefix = "", label, icon }) => {
    ...
    return (
        <div ref={ref} className="flex flex-col items-center justify-center p-6 text-center">
            ...
            <div className="text-lg text-[var(--color-cream)] font-medium">{label}</div>
        </div>
    );
};
...
const StaticStat = ({ icon, value, label }) => (
    <div className="flex flex-col items-center justify-center p-6 text-center">
        <div className="text-4xl mb-4">{icon}</div>
        <div className="text-3xl md:text-4xl font-serif font-bold text-[var(--color-gold)] mb-2">{value}</div>
        <div className="text-lg text-[var(--color-cream)] font-medium">{label}</div>
    </div>
);
