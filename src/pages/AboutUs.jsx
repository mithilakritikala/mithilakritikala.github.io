            ...
            <div className="container mx-auto px-4 max-w-6xl">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-24">
                    ...
                    <Reveal delay={120}>
                        ...
                        <blockquote className="border-l-4 border-[var(--color-gold)] pl-6 italic text-lg text-[var(--color-ink)]/80 mb-4" style={{ fontFamily: 'var(--font-display)' }}>
                            ...
                            <footer className="text-sm font-semibold text-[var(--color-maroon)] mt-2 not-italic" style={{ fontFamily: 'var(--font-body)' }}>— Vishal Kumar Thakur</footer>
                        </blockquote>
                ...
                <div className="mb-24">
                    <Reveal className="text-center mb-12">
                        <h2 className="text-3xl md:text-4xl font-semibold text-[var(--color-maroon)]">Our Journey</h2>
                    ...
                    <div className="relative max-w-3xl mx-auto">
                        ...
                        <div className="space-y-8">
                            {timeline.map((item, index) => (
                                <Reveal key={index} delay={index * 60} className="relative flex gap-6 items-start pl-2">
                                    ...
                                    <div className="bg-[var(--color-paper)] rounded-sm p-5 flex-grow shadow-sm border border-[var(--color-gold)]/20">
                                        <p className="text-[var(--color-terracotta)] font-bold text-sm uppercase tracking-widest mb-1">{item.date}</p>
                                        <p className="text-[var(--color-ink)]/80 leading-relaxed">{item.text}</p>
                                    </div>
                                </Reveal>
                            ))}
                        </div>
                ...
                <Reveal className="text-center mb-16">
                    <h2 className="text-3xl md:text-4xl font-semibold text-[var(--color-maroon)] mb-4">Meet Our Team</h2>
                    <div className="w-20 h-1 bg-[var(--color-gold)] mx-auto mt-4 mb-12"></div>
                </Reveal>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-24">
                    {[
                        { img: '/photos/founder.jpg', name: 'Vishal Kumar Thakur', role: 'Founder & Managing Trustee' },
                        { img: '/photos/trustii.jpg', name: 'Rakesh Kumar Thakur', role: 'Trustee' },
                        { img: '/photos/trustee.jpg', name: 'Babita Devi', role: 'Trustee & Training Head' },
                    ].map((person, idx) => (
                        <Reveal key={person.name} delay={idx * 100} className="bg-[var(--color-paper)] rounded-sm p-8 text-center shadow-sm">
                            <img src={person.img} alt={person.name} className="w-32 h-32 rounded-full mx-auto object-cover mb-6 shadow-md border-4 border-white" />
                            <h4 className="font-bold text-xl text-[var(--color-ink)] mb-2">{person.name}</h4>
                            <p className="text-[var(--color-maroon)] font-medium">{person.role}</p>
                        </Reveal>
                    ))}
                </div>
                ...
                {/* Our Values */}
                <Reveal className="text-center mb-12">
                    <h2 className="text-3xl md:text-4xl font-semibold text-[var(--color-maroon)] mb-4">Our Values</h2>
                    <div className="w-20 h-1 bg-[var(--color-gold)] mx-auto"></div>
                </Reveal>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {values.map((v, idx) => (
                        <Reveal key={v.title} delay={idx * 100} className="bg-white rounded-sm p-8 shadow-sm border-t-4 border-[var(--color-gold)]">
                            <div className="text-4xl mb-4">{v.icon}</div>
                            <h3 className="text-xl font-semibold text-[var(--color-maroon)] mb-3">{v.title}</h3>
                            <p className="text-[var(--color-ink)]/70 leading-relaxed">{v.desc}</p>
                        </Reveal>
                    ))}
                </div>
            </div>
